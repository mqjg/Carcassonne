...
class Deck:
    # TODO: Need to add ability to set the first card in the deck to the 
    #      starting tile.
    def __init__(self, deckFile="decks/vanillaDeck.json"):
        # TODO: we might be abel to get something similar to "Deck.__init__" in 
        #      a more generic way using __name__ to get the function name.
        ...
        if firstTile is None:
            ...
            if firstTile in self.tileList:
                ...
                for tileName in self.tileList:
                    ...
                    self.deck = self.deck + [Tile(tileDict,tileName) 
                                             for i in range(amount)]
...
class Tile:
    """
    carcosonne tiles are defined by their edges and center. The edges can be 
    roads, cities, or fields. The centers can be monasteries, crossroads, or 
    city entrances. Expansions can introduce other edges or centers. We call 
    these parts of the tile "features"

    You can divide the tiles edges into 12 portions (the center being a 13 
    portion). We represent this using a 13-element array of strings. For 
    example the starting tile would be:

        self.sides = [city, city, city, field, road, field, field, field, road, 
                      field, None]

    the last element of the list represents the center portion of the tile. 
    None indicates that there is no special center to the tile.

    This representation allows for easy accessing for tile compatibility 
    checks, but does not encode how tile elements (roads,cities, etc) are 
    connected. To store this information we keep a dictionary of the contents 
    of the tile. Each dictionary entry will be a 2-d nested list. Each each 
    element will be a list of integers indicating which of the edge are 
    connected. For example the starting tile would be:

        self.tileDict = {cities: [[0,1,2]], roads: [[10,4]], 
                         fields: [[11,3],[9,8,7,6,5]], monastery: [], 
                         cityEntrance: [], crossroad: []}

    TODO: Would a list of sets be better? Should we combine the center 
          entrys (monastery,cityEntrance,etc.) into just one entry called 
          "center" or something?
    """

    def __init__(self, tileDictInput, tileNameInput, 
                 imageDir="sprites/tileImages"):

        # TODO: This allows for portions of the tiles to overwritten and for 
        #      some portions of the tile to remain undefined. Undefined edges 
        #      will likely also be unrepresented in 
        ...
        # tile objects to the top, right, bottom, left sides of this tile in 
        # clockwise order
        self.neighbors = [None,None,None,None] 
        self.printStyles = ["pretty","verbose"]
        # this will become a 3d numpy integer array. The first two number will 
        # indicate the x,y position. The last digit will indicate the 
        # orientation of the tile (either 0,1,2,3 corresponding to rotating the 
        # tile 90 degrees clockwise)
        self.pos = None
        # Will be an integer to indicate the orientation of the tile 
        # (either 0,1,2,3 corresponding to rotating the tile 90 degrees 
        # clockwise)
        self.orient = 0 

        self.tileImage = pg.image.load(os.path.join(imageDir, self.tileName) \
                                       + ".png").convert()
        self.tileImage = pg.transform.scale(self.tileImage, 
    ...
    def visualizeTile(self,style="image", vpos=(0, 0)):
        ...
        if style == "text":
            # first letter of each side feature.
            s = [side[0] for side in self.sides] 
            print("  ____________","\n",
                  "| \ 0| 1 |2 /|","\n",
                  "|11\ |   | /3|","\n",
                  "|___\|___|/__|","\n",
...
class Map:
    ...
    def __init__(self, startingTile, mapCenter=np.array([0, 0])):
        ...
        # ordDirs, short for ordinal directions specifcy relative positions up, 
        # right, down, left (going clockwise) or a given location
        self.ordDirs = imageScale * np.array([[0, -1], [1, 0], 
                                              [0, 1], [-1, 0]])
        self.validPositions = self.ordDirs + mapCenter 
        print("")
    ...
    def getNeighbors(self, pos):
        ...
        # print(f"Map.getNeighbors: {neighborsPos[0]} {neighborsPos[1]} \
        #         {neighborsPos[2]} {neighborsPos[3]}")
    ...
    def validOrientation(self, newTile, neighbors):
        # TODO: range(len(neighbors)) will probably always be 4. Should I hard 
        # code it?
    ...
    def addTile(self, newTile, pos, orient):
        ...
        if self.validPosition(pos):
            ...
            if self.validOrientation(newTile,neighbors):
                ...
                # This recreates the list of valid positions.
...
class Button:
    ...
    def __init__(self, imageDir=os.path.join(os.getcwd(), "sprites/marker.png"),
                 pos=None):
        print(f"Button.__init__: creating button at {pos}")
        self.pos = pos
        self.buttonImage = pg.image.load(imageDir).convert()
        self.buttonImage = pg.transform.scale(self.buttonImage, 
                                              (imageScale, imageScale))
...
if __name__ == '__main__':
    ...
    # Make a window object? What other args are there?
    window = pg.display.set_mode(pageSize) 
    clock = pg.time.Clock()
    ...
    for i in range(deck.nTiles -1):
        ...
        buttons = [Button(imageDir=os.path.join(os.getcwd(), 
                          "sprites/squareMarker.png"), pos=validPosition) 
                   for validPosition in tileMap.validPositions]  
        ...
        while not tileAdded:
            # frames to render/second TODO: Does this need to go here?
            clock.tick(7.5)  
